...
function [E_area,E_prec,E_mean] = Plates2matOther2(plates,data,plates_hists,d,map,th_const,off_peak)
...
for p = 1:length(plates)
    ...
    for i = 1:8
        for j =1:12
            ...
            %For the gal80mig1 del use a well from the WT for normalization
            
            if p==1;
                y_off_other = xx{1}.bfp_yfp.f;
                x_off_other = xx{1}.bfp_yfp.xi;
            ...
            if~isempty(find(strcmp(names,map{i,j})))
                ...
                if length(xx{k}.other_yfp.f)>1
                    ...
                    E_mean{p}(n,m) = xx{k}.other_yfp.mean;
                    
                else
                    E_area{p}(n,m) = nan;
                    E_prec{p}(n,m) = xx{k}.other_yfp.perc_ind;
                    E_mean{p}(n,m) = xx{k}.other_yfp.mean;
                end
