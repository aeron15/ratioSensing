...
vmax_hxt = 10;      x(32) = vmax_hxt;
Khxt_glc = 1;     x(33) = Khxt_glc;
Khxt_gal = 250;     x(34) = Khxt_gal;

vmax_gal2 = 0.5*1;    x(35) = vmax_gal2;
Kgal2_glc = 2;    x(36) = Kgal2_glc;
...
r=1;
    ...
    for i = 1:length(agal_vec);
        ...
        gal_in_gal2 = vmax_gal2./(1 + Kgal2_gal./aglu + Kgal2_gal/Kgal2_glc*aglu./agal);
        
        gal_in = gal_in_hxt+gal_in_gal2;
        glc_in = glc_in_hxt+glc_in_gal2;
        
        [T Y] = ode15s(@(t,y) CompGalModelEqs(t,y,gal_in,0,1e7,x),[0 8*60],Y0); % Solve ODE
        G_vec(i,:) = Y(end,:);
    end
    %
    close all
    ...
    for i = 1:length(agal_vec);
        for j = 1:length(aglu_vec);
            ...
            gal_in(i,j) = gal_in_hxt+gal_in_gal2;
            glc_in(i,j) = glc_in_hxt+glc_in_gal2;
