...
home_name = pwd;
dir_name = 'C:\Users\ys151\Desktop\WorkingZone\Gal\LowOdGrowth_16_APR_2014\';
cd(dir_name);
filename = 'LowOdGrowth_16_APR_2014'
load(filename)
cd(home_name);
...
t = [t(:,1:12)];%;t(:,13:24)];
rate_avg = [rate_avg(:,1:12)];%(:,1:12);rate_avg(:,13:24)];
...
for i = 1:12
    ...
    s = fit(t(interval,i),log(counts_mat(interval,i)),'a*x+b','robust','on');
    m(i) = s.a;
    C(:,:,i) = confint(s,0.682);
    figure(5)
    subplot(4,3,i);
    plot(t(interval,i),log(counts_mat(interval,i)),'.');hold on;
...
save('C:\Users\ys151\Desktop\WorkingZone\Gal\LowODMeta\gal8hrs')
